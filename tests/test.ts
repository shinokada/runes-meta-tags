...
test('index page has expected h1, meta title', async ({ page }) => {
  await page.goto('/');
  await expect(page.getByRole('heading', { name: 'Svelte Icon Webkit Starter' })).toBeVisible();
...
test('Svelte 4 props page has expected h1, meta title', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Props: Svelte Supertiny v1' })).toBeVisible();
...
test('Svelte 5 props page has expected h1, meta title', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Props: Svelte Supertiny v2' })).toBeVisible();
